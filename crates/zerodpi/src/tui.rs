...
use zerodpi_core::proxy::{FindIpEvent, ProxyEvent, RelayEndReason};
...
pub enum DashboardInfo {
    ...
    /// Find-IP mode: show the selected domain and live IP pool.
    FindIp {
        sni: String,
        domain_ip: Ipv4Addr,
        max_ip: usize,
...
pub enum FindIpAction {
    /// User pressed 's' to stop and pick IP.
    StopAndPick,
    /// User pressed 'd' to change domain.
    ChangeDomain,
    /// User pressed 'r' to change IP range.
    ChangeRange,
...
fn draw_scan_progress(
    ...
    terminal.draw(|frame| {
        ...
        // Header
        let header = Paragraph::new("DeltaSpoof — Scanning SNIs…")
            .style(
                Style::default()
                    .fg(Color::Cyan)
...
fn draw_selection(frame: &mut ratatui::Frame, entries: &[SniProbeEntry], state: &mut TableState) {
    ...
    // Header
    let header = Paragraph::new("DeltaSpoof — Select SNI")
        .style(
            Style::default()
                .fg(Color::Cyan)
                .add_modifier(Modifier::BOLD),
    ...
    // Table
    let special_name = "www.hcaptcha.com";
    ...
    let rows: Vec<Row> = entries
        ...
        .flat_map(|(i, e)| {
            ...
            let is_special = e.sni == special_name;
            let rank_style = if is_special {
            ...
            rows.push(Row::new(vec![
                ...
                Cell::from(e.score.to_string()).style(score_style(e.score)),
                Cell::from(e.sni.clone()).style(sni_style),
                Cell::from(e.ip.to_string()),
                Cell::from(tcp_str).style(tcp_style(e.tcp_latency_ms)),
                Cell::from(tls_str).style(tls_style(e.tls_ok)),
    ...
    terminal.draw(|frame| {
        ...
        let header_lines = match info {
            DashboardInfo::SniSpoof { .. } | DashboardInfo::FindIp { .. } | DashboardInfo::AutoSpoof { .. } => {
                ...
                let (mode_label, second_line) = match info {
                ...
                let mut line2_spans = second_line;
                line2_spans.extend_from_slice(&[
                    Span::raw("   "),
...
pub fn run_ip_scan_progress(
...
) -> anyhow::Result<(Vec<IpProbeEntry>, bool, bool)> {
    ...
    loop {
        ...
        // Auto-start when enough candidates found.
        if min_candidates > 0 && arrived.len() >= min_candidates {
            return Ok((arrived, false, false));
        }
        ...
        if event::poll(Duration::from_millis(100))? {
            if let Event::Key(k) = event::read()? {
                if k.kind == KeyEventKind::Press {
                    match k.code {
                        KeyCode::Char('q') | KeyCode::Char('Q') | KeyCode::Esc => {
                            return Ok((arrived, true, false));
                        }
                        KeyCode::Char('r') | KeyCode::Char('R') => {
                            return Ok((arrived, false, true));
                        }
                        _ => {}
                    }
                }
...
fn draw_ip_scan_progress(
    ...
    terminal.draw(|frame| {
        ...
        let header = Paragraph::new("DeltaSpoof — Scanning IPs…")
            .style(
                Style::default()
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::BOLD),
...
fn draw_ip_selection(frame: &mut ratatui::Frame, entries: &[IpProbeEntry], state: &mut TableState) {
    ...
    let header = Paragraph::new("DeltaSpoof — Select IP")
        .style(
            Style::default()
                .fg(Color::Cyan)
                .add_modifier(Modifier::BOLD),
...
fn draw_sni_results_view(
    ...
    let title = format!("DeltaSpoof — SNI Scan Results ({} entries)", entries.len());
...
fn draw_ip_results_view(
    ...
    let title = format!("DeltaSpoof — IP Scan Results ({} entries)", entries.len());
    let header = Paragraph::new(title)
        .style(
            Style::default()
                .fg(Color::Cyan)
...
fn draw_proxy_scan_progress(
    ...
    terminal.draw(|frame| {
        ...
        // Header
        let header = Paragraph::new("DeltaSpoof — Proxy Scan: Phase 2 — Testing via VPN proxy…")
            .style(
                Style::default()
                    .fg(Color::Magenta)
                    .add_modifier(Modifier::BOLD),
...
fn draw_proxy_scan_results(
    ...
    let title = format!(
        "DeltaSpoof — Proxy Scan Results ({} tested, {} passed VPN)",
        entries.len(),
        passed,
    );
...
// ---------------------------------------------------------------------------
// CIDR range selection (for find_ip mode)
// ---------------------------------------------------------------------------

/// Show IP ranges from ip_list.txt and let the user select one CIDR range.
pub fn run_cidr_selection(
    terminal: &mut Term,
    ranges: &[ipnet::IpNet],
) -> anyhow::Result<ipnet::IpNet> {
    if ranges.is_empty() {
        anyhow::bail!("no CIDR ranges found in ip_list.txt");
    }
    ...
    loop {
        terminal.draw(|frame| {
            ...
            let header = Paragraph::new("DeltaSpoof — Select IP Range (find_ip)")
                .style(
                    Style::default()
                        .fg(Color::Cyan)
                        .add_modifier(Modifier::BOLD),
            ...
            let rows: Vec<Row> = ranges
                ...
                .map(|(i, net)| {
                    ...
                    Row::new(vec![
                        ...
                        Cell::from(format!("{host_count} IPs")),
                        Cell::from(if matches!(net, ipnet::IpNet::V4(_)) { "IPv4" } else { "IPv6" }),
                    ])
                })
                .collect();
            ...
            let table = Table::new(rows, widths)
                .header(
                    Row::new(vec!["#", "CIDR Range", "Host Count", "Version"])
                        .style(
                            Style::default()
                                .fg(Color::Cyan)
                                .add_modifier(Modifier::BOLD | Modifier::UNDERLINED),
                ...
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(" IP ranges from ip_list.txt "),
                )
                .row_highlight_style(
                    Style::default()
                        .bg(Color::Blue)
        ...
        if event::poll(Duration::from_millis(200))? {
            if let Event::Key(k) = event::read()? {
                ...
                match k.code {
                    ...
                    KeyCode::Down | KeyCode::Char('j') => {
                        let i = state.selected().unwrap_or(0);
                        state.select(Some((i + 1).min(ranges.len() - 1)));
                    }
                    KeyCode::Enter => {
                        let idx = state.selected().unwrap_or(0);
                        return Ok(ranges[idx]);
                    }
                    KeyCode::Esc | KeyCode::Char('q') | KeyCode::Char('Q') => {
                        return Ok(ranges[0]);
                    }
...
// ---------------------------------------------------------------------------
// Find-IP live dashboard
// ---------------------------------------------------------------------------

/// Show the live find_ip dashboard. Returns `Ok(None)` when user presses 's' or quits.
#[allow(clippy::too_many_arguments)]
pub fn run_find_ip_dashboard(
    terminal: &mut Term,
    ...
    loop {
        // Drain proxy events (fire-and-forget).
        loop {
        ...
        // Read live state from pool and counters.
        let (active_ips, cycle_active_count, is_fixed) = {
            let p = pool.read().unwrap();
            (p.active_ips().to_vec(), p.active_count(), p.is_fixed())
        };

        // Build per-IP stats from byte counters (per-cycle + total values).
        let mut ip_rows: Vec<(IpAddr, u64, u64, u64, u64, u64, Duration)> = Vec::new();
        {
            let mut p = pool.write().unwrap();
            p.update_cycle_counts(cfg.IP_TEST_TIMEOUT_SECS);
        }
        for &ip in &active_ips {
            ...
            ip_rows.push((ip, cycle_up, cycle_down, total_up + total_down, conns, cycles, duration));
        }
        ...
        if event::poll(Duration::from_millis(200))? {
            if let Event::Key(k) = event::read()? {
                if k.kind == KeyEventKind::Press {
                    match k.code {
                        KeyCode::Char('q') | KeyCode::Char('Q') | KeyCode::Esc => return Ok(FindIpAction::Quit),
                        KeyCode::Char('c') if k.modifiers.contains(KeyModifiers::CONTROL) => return Ok(FindIpAction::Quit),
                        KeyCode::Char('s') | KeyCode::Char('S') if !is_fixed => return Ok(FindIpAction::StopAndPick),
                        KeyCode::Char('p') | KeyCode::Char('P') => return Ok(FindIpAction::StopAndPick),
                        KeyCode::Char('d') | KeyCode::Char('D') => return Ok(FindIpAction::ChangeDomain),
                        KeyCode::Char('r') | KeyCode::Char('R') => return Ok(FindIpAction::ChangeRange),
                        _ => {}
                    }
                }
            }
...
fn draw_find_ip_live(
    ...
    terminal.draw(|frame| {
        ...
        if is_fixed {
            // Simplified FIXED mode: just show the selected IP.
            let header_lines = vec![
                Line::from(vec![
                    Span::styled("Mode: ", label_style()),
                    Span::styled("find_ip", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                    Span::raw("   "),
                    Span::styled("SNI: ", label_style()),
            ...
            // SCANNING mode: show all active IPs.
            let header_lines = vec![
                Line::from(vec![
                    Span::styled("Mode: ", label_style()),
                    Span::styled("find_ip", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                ...
                Line::from(vec![
                    ...
                    Span::styled(" to stop & pick IP", label_style()),
                ]),
            ];
            frame.render_widget(Paragraph::new(header_lines).block(Block::default().borders(Borders::ALL).title(" DeltaSpoof — Find IP Dashboard ")), chunks[0]);
            ...
            let rows: Vec<Row> = ip_rows.iter().map(|(ip, cup, cdown, total, conns, cycles, duration)| {
                ...
                Row::new(vec![
                    Cell::from(ip.to_string()).style(ip_style),
                    Cell::from(format!("{}/C", fmt_bytes(*cup))).style(Style::default().fg(Color::Cyan)),
                    Cell::from(format!("{}/C", fmt_bytes(*cdown))).style(Style::default().fg(Color::Green)),
                    Cell::from(fmt_bytes(*total)).style(Style::default().fg(Color::Yellow)),
                    Cell::from(conns.to_string()),
            ...
            let table = Table::new(rows, widths)
                ...
                .block(Block::default().borders(Borders::ALL).title(format!(" Active Connections ({}) ", ip_rows.len())));
            frame.render_widget(table, chunks[1]);
        }

        let help_line = if is_fixed {
            Line::from(vec![
                Span::styled(" p ", Style::default().fg(Color::Yellow)), Span::raw("pick new IP  "),
                Span::styled(" d ", Style::default().fg(Color::Yellow)), Span::raw("change domain  "),
                Span::styled(" r ", Style::default().fg(Color::Yellow)), Span::raw("change IP range  "),
                Span::styled(" q/Esc ", Style::default().fg(Color::Red)), Span::raw("quit "),
            ])
        } else {
            Line::from(vec![
                Span::styled(" s ", Style::default().fg(Color::Yellow)), Span::raw("stop & pick  "),
                Span::styled(" d ", Style::default().fg(Color::Yellow)), Span::raw("change domain  "),
                Span::styled(" r ", Style::default().fg(Color::Yellow)), Span::raw("change IP range  "),
                Span::styled(" q/Esc ", Style::default().fg(Color::Red)), Span::raw("quit "),
            ])
        };
        frame.render_widget(Paragraph::new(help_line).block(Block::default().borders(Borders::ALL)), chunks[2]);
    })?;
    Ok(())
}

// ---------------------------------------------------------------------------
// Top-5 IP selection (after pressing 's')
// ---------------------------------------------------------------------------

/// Show all pool IPs and let the user pick one.
pub fn run_top_ip_selection(
    terminal: &mut Term,
    pool: &std::sync::Arc<std::sync::RwLock<zerodpi_core::proxy::IpPool>>,
    byte_counters: &zerodpi_core::proxy::IpByteCounters,
    ...
    let mut all_ips: Vec<(IpAddr, u64, u64, u64, u64, u64, u64, u64)> = {
        let p = pool.read().unwrap();
        let mut ips: Vec<IpAddr> = byte_counters.upload.iter().map(|e| *e.key()).collect();
        for &ip in p.active_ips() {
            if !ips.contains(&ip) {
                ips.push(ip);
            }
        }
    ...
    loop {
        terminal.draw(|frame| {
            ...
            frame.render_widget(Paragraph::new("DeltaSpoof — Select Best IP").style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)).block(Block::default().borders(Borders::ALL)), chunks[0]);

            let rows: Vec<Row> = all_ips.iter().enumerate().map(|(i, (ip, _cup, _cdown, total_up, total_down, total, cycles, _))| {
                let rank_style = if i == 0 { Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD) } else { Style::default() };
                let total_style = if *total == 0 { Style::default().fg(Color::Red) } else { Style::default().fg(Color::Green) };
                Row::new(vec![
                    Cell::from((i + 1).to_string()).style(rank_style),
                    Cell::from(ip.to_string()),
                    Cell::from(fmt_bytes(*total_up)).style(Style::default().fg(Color::Cyan)),
                    Cell::from(fmt_bytes(*total_down)).style(Style::default().fg(Color::Green)),
                    Cell::from(fmt_bytes(*total)).style(total_style),
                    Cell::from(cycles.to_string()),
                ])
            }).collect();

            let table = Table::new(rows, [Constraint::Length(4), Constraint::Length(16), Constraint::Length(12), Constraint::Length(12), Constraint::Length(12), Constraint::Length(8)])
                .header(Row::new(vec!["#", "IP", "↑ Total", "↓ Total", "Total", "Cycles"]).style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD | Modifier::UNDERLINED)))
                .block(Block::default().borders(Borders::ALL).title(format!(" All Used IPs ({}) — sorted by Total ", all_ips.len())))
                .row_highlight_style(Style::default().bg(Color::Blue).fg(Color::White).add_modifier(Modifier::BOLD))
                .highlight_symbol("▶ ");
            frame.render_stateful_widget(table, chunks[1], &mut state);

            let help_spans: Line = Line::from(vec![
                Span::styled(" ↑/↓ or j/k ", Style::default().fg(Color::Yellow)), Span::raw("navigate  "),
                Span::styled(" Enter ", Style::default().fg(Color::Green)), Span::raw("select  "),
                Span::styled(" q / Esc ", Style::default().fg(Color::Red)), Span::raw("pick rank-1 "),
            ]);
            frame.render_widget(Paragraph::new(help_spans).block(Block::default().borders(Borders::ALL)), chunks[2]);
        })?;
        ...
        if event::poll(Duration::from_millis(200))? {
            if let Event::Key(k) = event::read()? {
                ...
                match k.code {
                    KeyCode::Up | KeyCode::Char('k') => { let i = state.selected().unwrap_or(0); state.select(Some(i.saturating_sub(1))); }
                    KeyCode::Down | KeyCode::Char('j') => { let i = state.selected().unwrap_or(0); state.select(Some((i + 1).min(all_ips.len() - 1))); }
