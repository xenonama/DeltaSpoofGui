...
fn packet_interception_access_error() -> &'static str {
    "DeltaSpoof needs Administrator privileges for packet interception via WinDivert. \
     Start PowerShell or Command Prompt with \"Run as administrator\" and run DeltaSpoof again. \
...
#[cfg(any(target_os = "linux", target_os = "android"))]
fn packet_interception_access_error() -> &'static str {
    "DeltaSpoof needs root privileges or CAP_NET_ADMIN for packet interception via NFQUEUE. \
     Run DeltaSpoof with sudo/root, grant CAP_NET_ADMIN to the binary, or use \
