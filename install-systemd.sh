...
find_deltaspoof_binary() {
    local dir="$1"

    if [ -f "$dir/deltaspoof" ]; then
        printf '%s\n' "$dir/deltaspoof"
        return 0
    fi
    ...
    while IFS= read -r -d '' candidate; do
        case "$(basename "$candidate")" in
            deltaspoof|deltaspoof-*) printf '%s\n' "$candidate"; return 0 ;;
        esac
...
BINARY_PATH="$(find_deltaspoof_binary "$APP_DIR")" || die "Could not find a DeltaSpoof executable in $APP_DIR."
...
cat > "$tmp_unit" <<EOF
[Unit]
Description=DeltaSpoof DPI bypass proxy
